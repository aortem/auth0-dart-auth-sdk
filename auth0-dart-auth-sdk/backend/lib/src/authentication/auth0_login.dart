...
class Auth0LoginException implements Exception {
  ...
  Auth0LoginException(this.message, {this.statusCode, this.errorCode});

  @override
  String toString() =>
      'Auth0LoginException: $message'
...
class Auth0LoginResponse {
  ...
  /// Parses a JSON response from Auth0 into an [Auth0LoginResponse] object
  factory Auth0LoginResponse.fromJson(Map<String, dynamic> json) {
...
class Auth0LoginService {
  ...
  ///
  /// Returns:
  ///   A [Future] that resolves to an [Auth0LoginResponse] on success
  ///
  /// Throws:
  ///   - [Auth0LoginException] for authentication failures
  ///   - [TimeoutException] if the request times out
  ///   - [FormatException] for malformed responses
  Future<Auth0LoginResponse> login(Auth0LoginRequest request) async {
    try {
      ...
      if (response.statusCode == 200) {
        return Auth0LoginResponse.fromJson(responseBody);
      } else {
        throw _parseErrorResponse(response.statusCode, responseBody);
      }
    } on TimeoutException {
      throw Auth0LoginException('Request timed out');
    } on FormatException {
      throw Auth0LoginException('Invalid server response format');
    } on http.ClientException catch (e) {
      // Handle ClientException without statusCode access
      throw Auth0LoginException('Network error: ${e.toString()}');
    } catch (e) {
      throw Auth0LoginException('Unexpected error: ${e.toString()}');
    }
  }

  /// Parses error responses from Auth0 into standardized exceptions
  Auth0LoginException _parseErrorResponse(
    int statusCode,
