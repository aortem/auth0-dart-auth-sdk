...
import '../exceptions/auth0_change_password_exception.dart';
import '../models/auth0_change_password_request_model.dart';
import '../models/auth0_change_password_response_model.dart';
...
class Auth0ChangePassword {
  ...
  ///
  /// Returns a [Future<Auth0ChangePasswordResponse>] containing Auth0's response.
  ///
  /// Throws [Auth0ChangePasswordException] if:
  /// - The request fails (non-200 status code)
  /// - The response cannot be parsed
  Future<Auth0ChangePasswordResponse> auth0ChangePassword(
    ...
    if (response.statusCode == 200) {
      ...
      try {
        if (response.headers['content-type']?.contains('application/json') ==
          ...
          return Auth0ChangePasswordResponse.fromJson(responseBody);
        } else {
          return Auth0ChangePasswordResponse(message: response.body);
        }
      } catch (e) {
        throw Auth0ChangePasswordException('Error parsing response: $e');
      }
    } else {
      // Handle error responses
      final errorMessage = _getErrorMessage(response);
      throw Auth0ChangePasswordException(errorMessage);
    }
  }
