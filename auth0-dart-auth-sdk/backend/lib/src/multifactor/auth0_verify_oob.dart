...
import '../models/auth0_verify_oob_request_model.dart';
import '../models/auth0_verify_oob_response_model.dart';
...
class Auth0MfaVerifyOob {
  ...
  /// Creates an instance of [Auth0MfaVerifyOob].
  ///
  ...
  Auth0MfaVerifyOob({required this.auth0Domain, http.Client? httpClient})
    : httpClient = httpClient ?? http.Client();

  /// Verifies the OOB code provided in the [request] by sending it to Auth0's token endpoint.
  ///
  /// If the verification is successful, it returns an [Auth0VerifyOOBResponse]
  ...
  Future<Auth0VerifyOOBResponse> verifyOOB(
    Auth0VerifyOOBRequest request,
  ) async {
    ...
    if (response.statusCode == 200) {
      try {
        ...
        return Auth0VerifyOOBResponse.fromJson(json);
      } catch (e) {
