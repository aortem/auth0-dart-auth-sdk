...
void main() {
  ...
  late Auth0DatabaseAdLdapPassive passiveAuth;
  const domain = 'test.auth0.com';
  const clientId = 'test_client_id';

  setUp(() {
    mockClient = MockClient();
    passiveAuth = Auth0DatabaseAdLdapPassive(
      domain: domain,
      clientId: clientId,
  ...
  group('Response Model', () {
    test('parses successful response', () {
      final response = Auth0DatabaseAdLdapPassiveResponse.fromJson({
    ...
    test('throws FormatException for invalid response', () {
      expect(
        () => Auth0DatabaseAdLdapPassiveResponse.fromJson({
          'token_type': 'Bearer',
        }),
