import 'package:auth0_dart_auth_sdk/src/models/auth0_authorize_endpoint_request_model.dart';
import 'package:auth0_dart_auth_sdk/src/models/auth0_authorize_endpoint_response_model.dart';
import 'package:auth0_dart_auth_sdk/src/authorization/auth0_authorize_endpoint.dart';
import 'package:ds_tools_testing/ds_tools_testing.dart';

void main() {
  group('Auth0AuthorizeEndpoint', () {
    final domainUri = Uri.parse('https://my-domain.auth0.com');

    test('throws ArgumentError when auth0DomainUri is empty', () {
      expect(
        () => Auth0AuthorizeEndpoint(auth0DomainUri: Uri.parse('')),
    ...
    test(
      ...
      () {
        final endpoint = Auth0AuthorizeEndpoint(auth0DomainUri: domainUri);

        final invalidRequest = Auth0AuthorizeEndpointRequest(
          clientId: '',
    ...
    test(
      'returns Auth0AuthorizeEndpointResponse with correct URL (minimum valid fields)',
      () {
        final endpoint = Auth0AuthorizeEndpoint(auth0DomainUri: domainUri);
        ...
        expect(response, isA<Auth0AuthorizeEndpointResponse>());
    ...
    test('returns Auth0AuthorizeEndpointResponse with all parameters', () {
      final endpoint = Auth0AuthorizeEndpoint(auth0DomainUri: domainUri);
