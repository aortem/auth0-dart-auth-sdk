import 'dart:convert';
import '../../../lib/src/models/auth0_challenge_request_request_model.dart';
import '../../../lib/src/multifactor/auth0_challenge_request.dart';
import 'package:ds_tools_testing/ds_tools_testing.dart';
import 'package:ds_standard_features/ds_standard_features.dart' as http;

void main() {
  group('Auth0MfaApiClient', () {
    const domain = 'https://example.auth0.com';

    test('returns Auth0ChallengeResponse on success', () async {
      ...
      final client = AuthOMultifactorChallengeRequest(
        auth0Domain: domain,
        httpClient: mockClient,
      );

      final response = await client.requestChallenge(
        Auth0ChallengeRequest(mfaToken: 'test-token'),
      );
    ...
    test('throws Exception on non-200 response', () async {
      ...
      expect(
        () => client.requestChallenge(
          Auth0ChallengeRequest(mfaToken: 'bad-token'),
        ),
        throwsA(
          predicate(
            (e) =>
    ...
    test('throws ArgumentError when mfaToken is missing', () async {
      final client = AuthOMultifactorChallengeRequest(auth0Domain: domain);
      expect(
        () => client.requestChallenge(Auth0ChallengeRequest(mfaToken: '')),
