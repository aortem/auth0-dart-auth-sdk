import 'dart:convert';
import '../../../lib/src/exceptions/auth0_accept_request_exception.dart';
import '../../../lib/src/models/auth0_accept_request_request_model.dart';
import '../../../lib/src/request_handling/auth0_accept_request.dart';
import 'package:ds_standard_features/ds_standard_features.dart' as http;
import 'package:ds_tools_testing/ds_tools_testing.dart';

void main() {
  group('Auth0AcceptRequestClient', () {
    ...
    test('should return valid response on HTTP 200', () async {
      ...
      final client = Auth0AcceptRequestClient(
        auth0Domain: testDomain,
        httpClient: mockClient,
      );

      final response = await client.acceptRequest(
        Auth0AcceptRequestRequest(clientId: 'abc123', ticket: 'sample-ticket'),
      );
    ...
    test(
      ...
      () async {
        final client = Auth0AcceptRequestClient(auth0Domain: testDomain);

        expect(
          () => client.acceptRequest(
            Auth0AcceptRequestRequest(clientId: '', ticket: ''),
          ),
    ...
    test('should throw custom exception on API failure', () async {
      ...
      expect(
        () => client.acceptRequest(
          Auth0AcceptRequestRequest(clientId: 'abc123', ticket: 'bad-ticket'),
        ),
        throwsA(
          isA<Auth0AcceptRequestException>().having(
            (e) => e.toString(),
            'contains error',
            contains('400'),
          ),
    ...
    test('should throw exception on unexpected error', () async {
      ...
      expect(
        () => client.acceptRequest(
          Auth0AcceptRequestRequest(clientId: 'abc123', ticket: 'ticket123'),
        ),
        throwsA(isA<Auth0AcceptRequestException>()),
      );
    });
  });
}
