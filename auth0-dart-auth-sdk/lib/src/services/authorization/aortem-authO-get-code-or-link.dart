...
import 'package:auth0_dart_auth_sdk/src/models/aortem_auth0_get_code_or_link_response.dart';
...
/// Example usage:
/// ```dart
/// final authService = AortemAuth0GetCodeOrLink(domain: 'your-auth0-domain');
...
/// }
/// ```
class AortemAuth0GetCodeOrLink {
  /// The Auth0 domain used for sending passwordless authentication codes or links.
  final String domain;

  /// Constructs an instance of [AortemAuth0GetCodeOrLink].
  ///
  ...
  AortemAuth0GetCodeOrLink({required this.domain}) {
  ...
  ///
  /// Returns a [AortemAuth0GetCodeOrLinkResponse] with the success message on a successful request.
  ///
  /// Throws an [Exception] if the request fails or if the response status is not 200.
  Future<AortemAuth0GetCodeOrLinkResponse> sendCodeOrLink({
    required AortemAuth0GetCodeOrLinkRequest request,
  }) async {
    ...
    if (response.statusCode == 200) {
      return AortemAuth0GetCodeOrLinkResponse(
        success: true,
