...
import 'package:auth0_dart_auth_sdk/src/logout/auth0_oidc_logout.dart';
import 'package:ds_tools_testing/ds_tools_testing.dart';

void main() {
  group('Auth0OidcLogout', () {
    const domain = 'example.auth0.com';
    final oidcLogoutService = Auth0OidcLogout(auth0Domain: domain);

    test('generates logout URL with only required clientId', () {
      final request = Auth0OidcLogoutRequest(clientId: 'abc123');

      final response = oidcLogoutService.generateLogoutUrl(request);

      expect(response, isA<Auth0OidcLogoutResponse>());
