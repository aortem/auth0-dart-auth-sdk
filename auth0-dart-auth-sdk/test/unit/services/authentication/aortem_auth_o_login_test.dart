...
import 'package:auth0_dart_auth_sdk/src/services/authentication/aortem-authO-login.dart';
  ...
  group('AortemAuth0LoginService', () {
    ...
    group('login', () {
      ...
      test('should include correct request body', () async {
        ...
        final actualBody = verify(() => mockHttpClient.post(
              any(),
      ...
      test('should throw AortemAuth0LoginException on non-200 response',
          () async {
        // Arrange
        final errorResponse = {
          'error': 'invalid_grant',
        ...
        expect(
          ...
          throwsA(isA<AortemAuth0LoginException>().having((e) => e.message,
              'message', 'Login failed: ${jsonEncode(errorResponse)}')),
        );
      });

      test('should throw AortemAuth0LoginException on http client exception',
          () async {
        // Arrange
        when(() => mockHttpClient.post(any(),
                headers: any(named: 'headers'), body: any(named: 'body')))
        ...
        expect(
          () => authService.login(validRequest),
          throwsA(isA<AortemAuth0LoginException>().having(
              (e) => e.message, 'message', 'Login failed: Network error')),
        );
      });
