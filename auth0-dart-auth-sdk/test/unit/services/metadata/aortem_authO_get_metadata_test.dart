import 'dart:convert';
import 'package:auth0_dart_auth_sdk/src/services/metadata/aortem-authO-get-metadata.dart';
  ...
  group('aortemAuth0GetMetadata40', () {
    ...
    test('returns metadata on success (200)', () async {
      final mockClient = MockClient((request) async {
        ...
        expect(
            request.url.toString(), '${auth0DomainUri.toString()}/metadata40');
        return http.Response(
            jsonEncode({'key1': 'value1', 'key2': 'value2'}), 200);
      });

      final request =
          AortemAuth0GetMetadata40Request(accessToken: 'test-token');
    ...
    test('throws on unauthorized (401)', () async {
      ...
      final request =
          AortemAuth0GetMetadata40Request(accessToken: 'invalid-token');
